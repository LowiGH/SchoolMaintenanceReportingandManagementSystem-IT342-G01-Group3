...
export function LoginPage() {
  ...
  const [validating, setValidating] = useState(true); // ✅ NEW: validation state
  ...
  // ✅ UPDATED:  Validate token with backend before redirecting
  useEffect(() => {
    const validateToken = async () => {
      ...
      try {
        // Create abort controller for 2-second timeout
        ...
        if (response.ok) {
          ...
          // ❌ Token is invalid (401, 403, etc.) - clear and show login
        ...
        // ❌ Network error or timeout - clear and show login
        console.log("❌ Token validation error (timeout or network):", error. message);
        localStorage.removeItem("authToken");
        localStorage. removeItem("userRole");
        localStorage.removeItem("user");
        setValidating(false);
  ...
  // ✅ NEW: Show loading spinner while validating token
  if (validating) {
    return (
